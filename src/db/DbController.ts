...
export class DbController {
  ...
  public async updateContent(contentModel: ContentModel): Promise<string> {
    return new Promise(async (resolve, reject) => {
      try {
        const content = await this.contentModel.findOne({ _id: contentModel._id });
        content.title = contentModel.title;
        content.cid = contentModel.cid;
        content.media = contentModel.media;
        content.url = contentModel.url;
        content.image = contentModel.image;
        content.vimeo = contentModel.vimeo;
        content.group = contentModel.group;
  ...
  public getContactAdmin(): Promise<ContactModel[]> {
    return new Promise((resolve, reject) => {
      try {
        this.contactModel.find({}, (err, res) => {
          if (err) return reject(err);
          resolve(res);
        })
      } catch (e) {
  ...
  public getContentId(id: string): Promise<ContentModel> {
    return new Promise((resolve, reject) => {
      try {
        this.contentModel.findById(id, (err, res) => {
          if (err) return reject(err);
          resolve(res);
